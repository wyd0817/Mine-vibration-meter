...
void Load_Drow_Dialog(void)
{
	...
	LCD_ShowString(296,224,200,16,16,"RST");//显示清屏区域
...
void TP_Save_Adjdata(void)
{
	...
	float a,b;	 
	//保存校正结果!
	a=tp_dev.xfac*100000000;//保存x校正因素      
	b=tp_dev.yfac*100000000;//保存y校正因素    

	TEXT_Buffer[0]=0x0A;//标记校准过了
	p1 = (u8 *)&a;//将浮点型a变为四个字节
	TEXT_Buffer[1] = *p1;
	p1++;
	TEXT_Buffer[2] = *p1;
	...
	p1++;
	TEXT_Buffer[8] = *p1;
	p1 = (u8 *)&tp_dev.xoff;//将短整形d变为两个字节  保存x偏移量
	TEXT_Buffer[9] = *p1;
	p1++;
	TEXT_Buffer[10] = *p1;
	p1 = (u8 *)&tp_dev.yoff;//将短整形e变为两个字节  保存y偏移量
	TEXT_Buffer[11] = *p1;
	p1++;
	TEXT_Buffer[12] = *p1;
	TEXT_Buffer[13] = tp_dev.touchtype;
...
{					  
	...
	tempfac=datatemp[0];//读取标记字,看是否校准过！	
	if(tempfac==0X0A)//触摸屏已经校准过了			   
	{    												 
		tempfac=*(float *)(datatemp+1);		   
		tp_dev.xfac=(float)tempfac/100000000;//得到x校准参数
		tempfac=*(float *)(datatemp+5);			          
		tp_dev.yfac=(float)tempfac/100000000;//得到y校准参数
	    //得到x偏移量
		tp_dev.xoff=*(short *)(datatemp+9);		   	  
 	    //得到y偏移量
		tp_dev.yoff = *(short *)(datatemp+11);				 	  
 		tp_dev.touchtype= datatemp[13];	//读取触屏类型标记
...
{								 
	...
	while(1)//如果连续50秒钟没有按下,则自动退出
	{
		...
		{	
			...
			{			   
				...
				case 4:	 //全部四个点已经得到
					...
					LCD_Clear(WHITE);//清屏
					LCD_ShowString(35,120,lcddev.width,lcddev.height,16,"Touch Screen Adjust OK!");//校正完成
					//TP_Adj_Info_Show(pos_temp[0][0],pos_temp[0][1],pos_temp[1][0],pos_temp[1][1],pos_temp[2][0],pos_temp[2][1],pos_temp[3][0],pos_temp[3][1],fac*100);//显示数据   
					//LCD_ShowString(40,260,200,16,16,"Press K2 to continue");	
					 
					//while(KEY_DOWN!=KEY_Scan(0));
					TP_Save_Adjdata();  					
					delay_ms(1000);
		...
		if(outtime>5000)
		{
			//TP_Get_Adjdata();
			LCD_Clear(WHITE);//清屏 
			LCD_ShowString(35,280,200,16,16,"Press K1 Adj Touch!");//显示清屏区域
			LED1 = 1;
			break;
	 	} 
...
{			    		   
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;				 // PB7端口配置  CS
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOB, &GPIO_InitStructure);
 	GPIO_SetBits(GPIOB,GPIO_Pin_7);//上拉

  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7|GPIO_Pin_5;	//PA7 - MOSI ,PA5 - TCK 端口配置	推挽输出 
 	GPIO_Init(GPIOA, &GPIO_InitStructure);
  GPIO_SetBits(GPIOA,GPIO_Pin_7|GPIO_Pin_5);//PA7, 上拉

 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;	// PA6端口配置  MISO
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //	上拉输入	
 	GPIO_Init(GPIOA, &GPIO_InitStructure);
	
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;	// PB6 端口配置 T_INT
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //	上拉输入
 	GPIO_Init(GPIOB, &GPIO_InitStructure);
