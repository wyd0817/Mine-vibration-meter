...
//mode:0,反白显示;1,正常显示  
//size:120 
void LCD_ShowBig_Num(u16 x,u16 y,u8 chr)
{   
	u8 temp,t1,t;
	u16 y0=y;
	u16 colortemp=POINT_COLOR;   	
	...
	for(t=0;t<120;t++)
	{   
		temp=asc2_6030[chr][t];                           
		for(t1=0;t1<8;t1++)
		{			
			if(temp&0x80)POINT_COLOR=WHITE;
			else POINT_COLOR=LIGHTBLUE;
			LCD_DrawPoint(x,y);	
			temp<<=1;
			y++;
			if(x>=lcddev.width){POINT_COLOR=colortemp;return;}//超区域了
...
u16 DANGER_POINT_COLOR=0x0000;	//画笔颜色
//x:0~319
//y:0~239
//mode:0,反白显示;1,正常显示  
//size:120 
void LCD_Show_XYZ(u16 x,u16 y,u8 chr)
{   
	u8 temp,t1,t;
	u16 y0=y;
	u16 colortemp=POINT_COLOR;   	
	...
	for(t=0;t<28;t++)
	{   
		temp=asc2_3015[chr][t];                           
		for(t1=0;t1<8;t1++)
		{			
			if(temp&0x80)POINT_COLOR=WHITE;
			else POINT_COLOR=DANGER_POINT_COLOR;
			LCD_DrawPoint(x,y);	
			temp<<=1;
			y++;
			if(x>=lcddev.width){POINT_COLOR=colortemp;return;}//超区域了
...
void LCD_ShowNum_2(u16 x,u16 y,u32 num,u8 len,u8 size)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_Show_XYZ(x+14*t,y,21);//空格
				//LCD_ShowChar(x+(size/2)*t,y,' ',size,0);
				continue;
			}else enshow=1; 
		 	 
		}
		LCD_Show_XYZ(x+14*t,y,temp);
	 	//LCD_ShowChar(x+(size/2)*t,y,temp+'0',size,0); 
	}
} 
